...
async fn proxy_handler(
    ...
    // Pass through relevant original headers
    ...
    match proxy_req.send().await {
        Ok(resp) => {
            ...
            tracing::debug!("[proxy] <- {} ({} headers)", status, resp.headers().len());
            ...
            for (name, value) in resp.headers() {
                let n = name.as_str().to_lowercase();
                // Skip CORS headers from target (Axum will add its own) 
                // and skip hop-by-hop headers
                if n != "cache-control" && 
